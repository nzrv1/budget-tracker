...
export function generateInsights(state: AppState): Insight[] {
  ...
  for (const goal of state.goals) {
    ...
    if (remaining <= 0) {
      ...
      insights.push({
        ...
        message: `Only ${remaining.toFixed(0)} ${state.settings.currency} left to reach "${goal.name}". At this pace you'll likely hit it before ${new Date(goal.targetDate).toLocaleDateString()}.`,
        createdAt: now.toISOString(),
      })
    } else if (daysLeft > 0) {
      ...
      if (currentWeeklySavingRate > 0 && currentWeeklySavingRate < neededPerWeek * 0.6) {
        insights.push({
          ...
          message: `You'd need to save about ${neededPerWeek.toFixed(0)} ${state.settings.currency}/week to hit this goal by its target date — your current pace looks slower than that.`,
          createdAt: now.toISOString(),
        })
      }
      ...
      insights.push({
        ...
        message: `The target date for "${goal.name}" has passed with ${remaining.toFixed(0)} ${state.settings.currency} still needed. Consider adjusting the date or the target amount.`,
        createdAt: now.toISOString(),
      })
    }
  }
  ...
  if (avgWeeklySpend > 0 && weekExpense > avgWeeklySpend * 1.3) {
    insights.push({
      ...
      message: `This week's spending (${weekExpense.toFixed(0)}) is running well above your typical weekly pace (${avgWeeklySpend.toFixed(0)}) this month.`,
      createdAt: now.toISOString(),
    })
  }
